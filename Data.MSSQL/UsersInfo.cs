...
namespace Data.MSSQL
{
    public class UsersInfo : Data.Interface.IUsersInfo
    {
        ...
        public int Add(Data.Model.UsersInfo model)
        {
            string sql = @"INSERT INTO UsersInfo
				(UserID,Officer,Tel,Fax,Address,Email,QQ,MSN,Note,RestDate,PublicDate,LastPublicDate) 
				VALUES(@UserID,@Officer,@Tel,@Fax,@Address,@Email,@QQ,@MSN,@Note,@RestDate,@PublicDate,@LastPublicDate)";
            SqlParameter[] parameters = new SqlParameter[]{
                ...
                new SqlParameter("@RestDate", SqlDbType.Decimal, -1) { Value = model.RestDate},
                new SqlParameter("@PublicDate", SqlDbType.Decimal, -1) { Value = model.PublicDate},
                 new SqlParameter("@LastPublicDate", SqlDbType.Decimal, -1) { Value = model.LastPublicDate}
			};
	...
	Officer=@Officer,Tel=@Tel,Fax=@Fax,Address=@Address,Email=@Email,QQ=@QQ,MSN=@MSN,Note=@Note,RestDate=@RestDate,PublicDate=@PublicDate,LastPublicDate=@LastPublicDate 
WHERE UserID=@UserID";
            SqlParameter[] parameters = new SqlParameter[]{
				model.Officer == null ? new SqlParameter("@Officer", SqlDbType.NVarChar, 1000) { Value = DBNull.Value } : new SqlParameter("@Officer", SqlDbType.NVarChar, 1000) { Value = model.Officer },
				model.Tel == null ? new SqlParameter("@Tel", SqlDbType.VarChar, 500) { Value = DBNull.Value } : new SqlParameter("@Tel", SqlDbType.VarChar, 500) { Value = model.Tel },
        ...
        private List<Data.Model.UsersInfo> DataReaderToList(SqlDataReader dataReader)
        {
            ...
            while (dataReader.Read())
            {
                ...
                if (!dataReader.IsDBNull(8))
                    ...
                    model.RestDate  = dataReader.GetDecimal(9);
                    model.PublicDate = dataReader.GetDecimal(10);
                    model.LastPublicDate = dataReader.GetDecimal(11);
                List.Add(model);
            }
            return List;
        }
