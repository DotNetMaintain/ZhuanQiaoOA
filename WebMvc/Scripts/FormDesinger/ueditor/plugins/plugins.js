...
//物资名称选择框
UE.plugins['formMaterial'] = function () {
    var me = this, thePlugins = 'formMaterial';
    me.commands[thePlugins] = {
        execCommand: function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialogs/MaterialList.aspx',
                name: thePlugins,
                editor: this,
                title: '物资名称选择框',
                cssRules: "width:500px;height:280px;",
    ...
    me.addListener('mouseover', function (t, evt) {
        ...
        var type1 = el.getAttribute('type1');
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', '')) {
            var html = popup.formatHtml('<nobr>物资名称选择框: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html) {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor(popup.anchorEl);
